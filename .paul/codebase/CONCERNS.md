...
- ✅ **Discovery (was `DRAFT_NAME_RE` in `lcc_core/profile_registry.py`) — fixed in the current WIP.** Replaced by `_is_draft_model()`, which treats `mtp` as a companion only when it is a path segment, a filename *prefix* (`mtp-`), or an `-mtp.gguf` suffix. Its docstring calls out the exact former failure: "``-MTP-`` in the middle of a product name (e.g. NVFP4-MTP-Q8attn) is not." **This exists only in the working tree — `git show HEAD:lcc_core/profile_registry.py` still has the old regex.** If that WIP is ever discarded, the bug returns.
